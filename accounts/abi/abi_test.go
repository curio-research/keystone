...
func TestAbiPackValHelperFunctions(t *testing.T) {

	testStr := "Hello"
	packedStr, _ := PackString(testStr)
	unpackedStr, _ := UnpackString(packedStr)
	if unpackedStr != testStr {
		t.Fatal("Pack string failed")
	}

	testUint256 := int64(420)
	packedUint256, _ := PackUint256(testUint256)
	unpackedUint256, _ := UnpackUint256(packedUint256)

	if unpackedUint256 != testUint256 {
		t.Fatal("Pack uint256 failed")
	}

	testAddress := common.Address{0x01}
	packedAddress, _ := PackAddress(testAddress)
	unpackedAddress, _ := UnpackAddress(packedAddress)

	if unpackedAddress != testAddress.String() {
		t.Fatal("Pack address failed")
	}

	packedBoolean, _ := PackBool(true)
	unpackedBoolean, _ := UnpackBool(packedBoolean)
	if unpackedBoolean != true {
		t.Fatal("Pack bool failed")
	}

	// test ABI encode positions
	testPosition := BigIntPos{X: big.NewInt(1), Y: big.NewInt(2)}

	packed, _ := PackPosition(testPosition)
	unpacked, _ := AbiDecodePositionFromBytes(packed)

	if testPosition.X.Cmp(unpacked.X) != 0 || testPosition.Y.Cmp(unpacked.Y) != 0 {
