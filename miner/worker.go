...
func (w *worker) taskLoop() {
	...
	for {
		...
		case task := <-w.taskCh:
			...
			// task created from here to "seal block". need to trace even before this
...
func (w *worker) resultLoop() {
	...
	for {
		...
		case block := <-w.resultCh:
			...
			// ----------------------------------------------------------------
			// treaty approveMove() hooks

			currentState := task.state.Copy()

			latestWorld := task.state.GetLatestWorldState()

			treatyEntitiesQuery := []engine.QueryCondition{{QueryType: engine.Has, Component: game.TagComp, Value: "Treaty"}}
			treatyEntities := latestWorld.QueryAsArray(treatyEntitiesQuery)

			// treaty address -> entity number
			addressToEntity := map[common.Address]int64{}

			// get the address of each treaty
			allTreatyAddresses := []common.Address{}
			for _, treatyEntity := range treatyEntities {
				addressString, ok := (latestWorld.GetComponentValue(engine.AddressComp, treatyEntity)).(string)

				if !ok {
					return
				}

				hexAddress := common.HexToAddress(addressString)

				allTreatyAddresses = append(allTreatyAddresses, hexAddress)
				addressToEntity[hexAddress] = treatyEntity
			}

			// 🚧 Demo: Apply stateless treaty hooks here 🚧

			// here, treaty hooks restrict movement
			// we parallelize calling all treaty contracts for all entities for approving move
			// treaty smart contracts here return boolean value determining whether the entity can move
			// in this case, since all Treaty functions are view only, they are safe to parallelize

			wg := sync.WaitGroup{}

			failedMovementEntities := []int64{}

			for i := 0; i < len(allTreatyAddresses); i++ {
				treatyAddress := allTreatyAddresses[i]

				wg.Add(1)
				go func(address common.Address) {

					// TODO: somehow make sure the contract is a view only function

					sender := vm.AccountRef(common.BytesToAddress([]byte{0})) // since they're view function we'll call them from a empty address
					txContent := vm.TxContext{Origin: sender.Address(), GasPrice: new(big.Int)}
					blockContext := vm.BlockContext{BlockNumber: w.chain.CurrentBlock().Number, Time: 0, Coinbase: common.Address{}, GasLimit: 1000000000, Difficulty: big.NewInt(0)}
					tempEvm := vm.NewEVM(blockContext, txContent, currentState, w.chainConfig, *w.chain.GetVMConfig())

					res, _, callErr := tempEvm.Call(sender, address, nil, 1000000000, new(big.Int))

					entity := addressToEntity[address]

					// TODO: add error handling
					treatyResult, _ := abi.UnpackBool(res)

					// cast return type as boolean
					if treatyResult == false || callErr != nil {
						failedMovementEntities = append(failedMovementEntities, entity)
					}
			...
			// get all entities that need to be moved
			targetEntitiesQuery := []engine.QueryCondition{
				{QueryType: engine.Has, Component: engine.TargetPositionComp, Value: 0},
			}
			movableEntities := latestWorld.QueryAsArray(targetEntitiesQuery)

			// remove entities that failed to move due to treaty violations
			movableEntities = engine.Filter2Arrays(movableEntities, failedMovementEntities)

			task.state.MovableEntities = movableEntities
...
func encodeMove(functionName string) ([]byte, error) {
	// Create a new instance of the ABI for the contract
	myABI, err := abi.JSON(strings.NewReader(approveMoveAbi))
	...
	if w.isRunning() {
		...
		if !w.isTTDReached(block.Header()) {
			...
			case w.taskCh <- &task{receipts: env.receipts, state: env.state, block: block, createdAt: time.Now()}:
				...
				new(big.Float).Quo(new(big.Float).SetInt(fees), big.NewFloat(params.Ether))
