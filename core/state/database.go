...
// presists through blocks.
type cachingDB struct {
	// DB store here?
	disk          ethdb.KeyValueStore
	codeSizeCache *lru.Cache[common.Hash, int]

	// setStateObject and getStateObject uses this to fetch the previous contract code from the cache.
	// this means that the state reads are quite fast!
	...
	// make a database here of structs
	// store game world

	// should we place the snapshottable state somewhere else?

	// worlds     map[int64]engine.World // blockNumber -> world state
	// worldNonce int64
}
