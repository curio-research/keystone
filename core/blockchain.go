...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(header *types.Header) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	// Curio Customized Initial Phase
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) error {
	// println("inside writeBlockWithState()")
	...
	// TODO: this commits state from memory to disk
	if bc.cacheConfig.TrieDirtyDisabled {
...
func (bc *BlockChain) WriteBlockAndSetHead(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	// println("Writing block and set heading")
	if !bc.chainmu.TryLock() {
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		// FIXME: it reinitializes a new one

		println("⛓ Creating a new chain in blockchain.go")
		...
		fmt.Println("📝 Writing block to the chain")
