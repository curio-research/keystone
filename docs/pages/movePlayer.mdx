...
1. Just like we did in the  `CreatePlayerSystem` , we `Filter` for players with the playerID, but this time to check whether the player the user is referring to exists.
2. `ValidateTileToMoveTo` confirms across different schemas (using, surprise, `Filter` !) that an object doesn’t exist at the position we are seeking to go to.
3. We pick up resources by:
...
And this is the syntax for adding a system:
